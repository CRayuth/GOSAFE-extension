(() => {
  ...
  /** @typedef {{ id: string, ts: number, kind: string, title: string, detail?: string, host?: string, count?: number, ruleset?: string, initiator?: string, url?: string, type?: string }} ActivityEntry */
  ...
  class ActivityLogStore {
    ...
    /**
     * Network / DNR block — coalesce bursts per host.
     * @param {{ url?: string, initiator?: string, rulesetId?: string, type?: string }} info
     */
    async recordBlock(info = {}) {
      ...
      const merged = this._buf.mergeRecent(
        ...
        (item) => {
          ...
          item.title = `Blocked ${host} ×${item.count}`;
          item.detail = initiator ? `from ${initiator}` : ruleset;
      ...
      if (!merged) {
        this._buf.pushFront(
          ActivityLogStore.normalize({
            kind: "blocked",
            title: `Blocked ${host}`,
            detail: initiator ? `from ${initiator}` : ruleset,
            host,
            ruleset,
            initiator,
            url,
